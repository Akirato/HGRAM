...
def main():
    ...
    config = [('HyperGraphConv', [feat[0].shape[1], args.hidden_dim])]

    if args.h > 1:
        config = config + [('HyperGraphConv', [args.hidden_dim, args.hidden_dim])] * (args.h - 1)
    ...
    for epoch in range(args.epoch):
        ...
        if accs[-1] > max_acc:
            max_acc = accs[-1]
            model_max_state = copy.deepcopy(maml.state_dict())
    ...
    model_max = maml
    model_max.load_state_dict(model_max_state)
    for x_spt, y_spt, x_qry, y_qry, c_spt, c_qry, n_spt, n_qry, g_spt, g_qry in db_t:
    ...
    print('Max Memory:', str(max_memory)[:5])
